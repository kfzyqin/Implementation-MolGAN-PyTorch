...
class Encoder(nn.Module):
    ...
    def __init__(self, conv_dim, m_dim, b_dim, z_dim, dropout):
        super(Encoder, self).__init__()

        graph_conv_dim, aux_dim, linear_dim = conv_dim
        # linear_dim.append(z_dim)
        # discriminator
        ...
        layers.append(nn.Linear(linear_dim[-1], z_dim))
        self.linear_layer_mu = nn.Sequential(*layers)
        self.linear_layer_logdev = nn.Sequential(*layers)
    ...
    def forward(self, adj, hidden, node, activatation=None):
        ...
        h_mu = self.linear_layer_mu(h)
        h_logdev = self.linear_layer_logdev(h)

        h = self.reparameterize(h_mu, h_logdev)

        return h, h_mu, h_logdev
